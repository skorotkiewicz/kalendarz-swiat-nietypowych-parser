...
import { CACHE_DB } from "./wikiDownloader.js";
...
const manualDates = [
  "data ustalana",
  "marzec lub kwiecień",
  "równonoc wiosenna", // @todo handle równonoc
...
const parseMonth = ($, $list, _title, YEAR) => {
  ...
  return Array.from(
    $list.map(function () {
      ...
      const monthDay = Number(
        $item.find("a").attr("title").split(" ")[0].trim(),
      );
...
const findDate = (text, YEAR) => {
  ...
  switch (text) {
    ...
    case "Przeddzień środy popielcowej": {
      const ashDay = specials.ashDay(YEAR);
      return findDateBefore(
        ashDay,
        ashDay.dayOfWeek === 1 ? 7 : ashDay.dayOfWeek - 1,
      );
    }
...
const specials = {
  easter: (Y) => {
    // thanks SO: https://stackoverflow.com/a/1284335
    ...
    return getDateResult(new Date(Y, M - 1, D));
  },
  ashDay: (year) => {
...
const parseMovable = ($, $list, _title, YEAR) =>
  Array.from(
    $list.map(function () {
      const $item = $(this);
      const text = entities.decode($item.text());
...
const parse = (html, YEAR) => {
  ...
  $headers.each(function () {
    ...
    if (parsedItems) {
      for (const item of parsedItems) {
        ...
        for (const dayName of item.list) {
          list.push({
            month: item.month,
            day: item.day,
            year: item.year,
  ...
  fs.writeFile(CACHE_DB, JSON.stringify(list))
    .then(() => console.info("Saved db to cache"))
    .catch(() => console.error("Couldn't store db to cache"));
